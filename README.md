...
## Docker, Local LLM & CI/CD Integration

On top of the AI client integration above, this repo adds three new capabilities.
Full guide is in [`INTEGRATIONS.md`](INTEGRATIONS.md); a step-by-step summary is below.
...
### A. Build & run via Docker

There are **three** purpose-built images:
...
> `cap_add: NET_RAW/NET_ADMIN` is already set in compose for tools like
> `nmap -sS`, `masscan`, `arp-scan`.

---

### B. Local OpenAI-compatible LLM integration (vLLM)

HexStrike's "brain" lives on the client side. This agent uses **your local model**
to run 100+ tools — without Claude/Cursor.

**Steps:**

```bash
# 1. Start the vLLM server with function-calling ENABLED
...
# 2. Configure
cp .env.example .env
#   edit: OPENAI_BASE_URL, OPENAI_MODEL, OPENAI_API_KEY
...
> Llama-3.1-Instruct, Mistral, etc.). Adjust `--tool-call-parser` per model.

---

### C. Use HexStrike as a security gate in CI/CD

Two stages: **check the code before deploy** and **pentest after deploy**.
The gate is **deterministic** (based on real tool findings); the `--use-llm`
option adds a triage summary from vLLM (advisory).

| Stage | Command | Tools | Gate |
|-------|---------|-------|------|
| Pre-deploy (SAST) | `hexstrike_ci.py code-scan` | trivy fs, checkov, semgrep, gitleaks | fails if findings ≥ `--fail-on` |
| Post-deploy (DAST) | `hexstrike_ci.py pentest` | wafw00f, httpx, nuclei, nikto | fails if findings ≥ `--fail-on` |

**Quick local test:**
```bash
python3 hexstrike_server.py --port 8888 &
# Scan the current repo (fails on HIGH/CRITICAL findings)
python3 hexstrike_ci.py --fail-on high code-scan --path .
# Pentest a deployed URL
python3 hexstrike_ci.py --fail-on high pentest --target https://staging.example --profile quick
# Reports -> hexstrike-reports/hexstrike-{code-scan,pentest}.{json,md}
```

**Wire it into your GitLab application pipeline:**

1. Build & push the HexStrike images to your private registry (the
   [`.gitlab-ci.yml`](.gitlab-ci.yml) pipeline; set CI/CD Variables
   `REGISTRY_IMAGE`, `REGISTRY`, `REGISTRY_USER`, `REGISTRY_PASSWORD`).
2. In your **application** `.gitlab-ci.yml`, include the template:
   ...
   variables:
     ...
     HEXSTRIKE_USE_LLM: "false"

   hexstrike_pentest:
     needs: ["deploy_staging"]     # run after your deploy job
   ```

3. Jobs provided by the template:
   - `hexstrike_code_scan` (stage `test`) — uses the **pre-deploy** image
   - `hexstrike_pentest` (stage `dast`) — uses the **post-deploy** image
   - `lint_python` / `lint_js` / `lint_go` / `lint_shell` / `lint_dockerfile`
     (stage `test`) — **code quality**, auto-enabled per language (`rules: exists`),
     advisory (`allow_failure: true`; override to `false` to block).

Reports are stored as artifacts under `hexstrike-reports/`.

> **Scope note:** HexStrike covers the **security** side (dependency CVEs,
> secrets, IaC misconfig, semgrep SAST, DAST). Pure *code quality* is handled by
> the linter jobs above — HexStrike is not a replacement for SonarQube.

---

### D. Trigger a scan/pentest via Webhook (post-deploy)

The post-deploy image bundles [`hexstrike_webhook.py`](hexstrike_webhook.py):
a service to **trigger scans on demand** with a **target (domain/IP)** parameter
and **optional creds** (authenticated scan) — ideal to be called by your CD
system after a deploy.
...
curl -X POST http://localhost:9000/trigger \
  ...
  -H "Content-Type: application/json" \
  -d '{
        "target": "https://staging.my-app.example",
        "action": "pentest",
        "profile": "quick",
        "fail_on": "high"
      }'
# -> {"job_id":"abcd1234...","status":"accepted"}

# 3. Check status & findings summary
curl -H "X-Webhook-Token: $WEBHOOK_TOKEN" http://localhost:9000/status/abcd1234...
```

**Authenticated scan** (send creds; not logged, not visible in the process list):
```jsonc
{
  "target": "https://app.example",
  "action": "pentest",
  "auth": { "type": "bearer", "token": "eyJ..." }
  // or: {"type":"basic","username":"u","password":"p"}
  //     {"type":"cookie","cookie":"sid=...; csrf=..."}
  //     {"type":"header","headers":["X-Api-Key: k"]}
}
```

Webhook security:
- You must set `WEBHOOK_TOKEN` (header `X-Webhook-Token`) **or** `WEBHOOK_HMAC_SECRET`
  (verifies `X-Hub-Signature-256`, GitHub-style). Without either, `/trigger` rejects everything.
- The target is validated (rejects shell metacharacters); only run against assets you are authorized to test.
- Endpoints: `POST /trigger`, `GET /status/<job_id>`, `GET /jobs`, `GET /health`.

---

### E. Remote / endpoint mode — HexStrike on a VPS, called from CI

Run the pre-deploy and post-deploy images as **long-running services on a VPS**
and have your GitLab runner just **call the endpoints** (jobs run in a tiny
`alpine` image — no HexStrike image inside the pipeline).

```bash
# On the VPS: run both endpoint services (deploy/docker-compose.vps.yml)
...
#   code-scan -> :9001/scan/code   (upload repo archive)
#   pentest   -> :9000/trigger     (deployed URL)
```

In your application `.gitlab-ci.yml`, include the **remote** template
